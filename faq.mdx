...
Any S3-compatible bucket you own. The docs use Cloudflare R2. Other S3-compatible stores, such as MinIO, work with the same settings plus `ROWSAFE_REPO_S3_PORT` or `ROWSAFE_REPO_S3_CA_FILE` if they need them. Storage is billed by your provider, not by Rowsafe.
