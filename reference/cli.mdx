...
| `rowsafe recommendations [NAME]` | [Index recommendations](/docs/guides/index-recommendations): indexes that make your busiest queries faster, each tested on a copy of the database, with the speedup per query, size and effect on writes. `--all` adds created, hidden and no longer needed ones; `--json`. Create one with `rowsafe fix`. |
| `rowsafe recommendations find [NAME]` | Look for index recommendations now (they run every night by itself) and wait for the result. `--no-wait`. |
| `rowsafe recommendations schedule [NAME] auto\|off\|CRON` | When to look: `auto` (every night at the quietest hour), `off`, or a 5-field cron expression in UTC. |
