...
| **Admin** | Everything a member can, plus adopt databases, run tasks, restart PostgreSQL (on servers that allow it), manage hosts, alert rules, API keys and members. |
...
It records, among others: databases added, changed and removed; hosts changed and removed; enrollment tokens created; tasks queued (backups, Proof runs, plans, restarts); forks (`database.fork`); Marks; API keys created and revoked; alerts acknowledged; alert rules and notification channels changed; plan changes; and CLI logins approved or denied.
