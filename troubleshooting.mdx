...
The plan was applied, but `archive_mode` only takes effect after a restart, and Rowsafe never restarts PostgreSQL on its own. Restart it when it suits you:

```sh
sudo systemctl restart postgresql@18-main     # your cluster's unit; in Docker: docker compose restart postgres
```

**Rowsafe notices the restart by itself** and finishes: it checks that changes reach your bucket, and the first full backup starts. If the database still says it is awaiting a restart a few minutes later, click **Check now** on the database's page, or run:

```sh
rowsafe verify app
```

If you allowed restarts during install, you can instead click **Restart PostgreSQL now** on the database's page, or run `rowsafe restart app`. To get that button on a server where you didn't allow it, run the installer again with `--allow-restart`:
