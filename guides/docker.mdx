...
- Backups, checks, Marks, Proof (the weekly restore test) and Rewind copies run in the agent container. It reads the data volume **read-only**, at the same path.
...
- **Rewinding the whole database in place isn't available in Docker.** Rowsafe never stops or restarts the PostgreSQL container, and the agent sees the data volume read-only. Restore a copy and bring back the rows you need instead, or [restore by hand](/docs/guides/restore#advanced-restore-by-hand-or-to-another-server) into a new volume.
