---
title: CLI reference
description: Every rowsafe command, its short form, flags, exit codes, environment variables and the .rowsafe.json project file.
---
...
| `rowsafe mark [NAME] [LABEL]` | | Create a [restore point](/docs/concepts/restore-points). `LABEL` defaults to `manual-<UTC time>`. |
...
| `rowsafe health [NAME]` | Health score (0-100) and findings with what to do. Without NAME, every database, worst first. Exit code 3 if one scores below 70. `--json`. |
...
| `rowsafe report` | The weekly email: show settings, `--preview` (`--html`), `--on`/`--off`, `--to A,B`, `--send-test`. |
...
| Command | What it does |
|---|---|
| `rowsafe mcp` | [MCP server](/docs/reference/mcp) on stdio, read-only. `--allow-restore-points` adds restore points; `--allow-writes` adds every write tool. |
| `rowsafe guard` | The Claude Code `PreToolUse` hook: creates a restore point before a destructive database command. See [AI agents](/docs/guides/ai-agents#the-guard-hook). |
