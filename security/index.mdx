...
- runs **only a fixed set of tasks**: inspect, adopt (plan or apply), check, backup, drill (Proof), restore point (Mark), and restart, only on a server where you allowed it. It never runs commands or SQL sent over the network.
...
- restart PostgreSQL on a server where you didn't allow it, stop PostgreSQL, delete data, or change settings other than the archiving settings above.
