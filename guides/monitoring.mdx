...
Rowsafe watches your databases with no monitoring stack of your own. The agent collects database and server metrics every minute, and Rowsafe turns them into a **health score**, plain-language findings, graphs, alerts and a weekly email.

Monitoring starts as soon as you add a database with `rowsafe adopt`, before backups are switched on. Monitoring is included in every plan, including Free.
...
Every Monday morning, "Your databases this week" arrives by email: each database's health score and how it changed, backups and restore tests of the week, size and growth, the disk forecast, the top things to fix, the slowest queries, and any open alerts.
...
You can change the same settings in the dashboard under **Settings**, then **Notifications**.
...
Backup and archiving rules (`backup_stale`, `wal_archiving_failing`, `postgres_unreachable`, drill rules, `restore_point_unconfirmed`) wait until backups are switched on for the database, so a database you just added doesn't page you. Disk, connection, wraparound, lock and replication rules apply from the start.
