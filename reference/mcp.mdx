...
| `index_recommendations` | read | Indexes Rowsafe tested on a copy of the database, with the speedup per query, size and effect on writes, and the indexes it created. Creating one is for a person (Apply fix). |
...
| Tool | Parameters |
|---|---|
| `get_database`, `safety_check`, `list_restore_points`, `index_recommendations` | `database` (required) |
