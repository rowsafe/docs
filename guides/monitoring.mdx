...
description: Pulse is Rowsafe's monitoring. The health score, what the agent collects, the built-in alert rules, the weekly email, and how to send alerts to email, Slack, Discord or a webhook.
---

**Pulse** watches your databases, with no monitoring stack of your own. The agent collects database and server metrics every minute, and Rowsafe turns them into a **health score**, plain-language findings, graphs, alerts, slow-query insights and a weekly email, "Your weekly Pulse".
...
Every database gets a score from **0 to 100** and a short list of findings, worst first. Each finding says what's wrong in plain language, why it matters and what to do, often with the exact command.
