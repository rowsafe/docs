---
title: Use Rowsafe with AI agents
description: A safety net for coding agents that run migrations and SQL. Check protection, create a restore point, then proceed.
---
...
Rowsafe gives you that moment. Before a risky change, the agent checks that the database is recoverable and creates a named [restore point](/docs/concepts/restore-points). If something breaks, you know exactly where to restore to.
...
3. **Proceed.**
4. **If something breaks, stop.** The agent doesn't try to repair data and never restores anything itself. It tells you the restore point, and you decide whether to [restore](/docs/guides/restore#to-a-restore-point).
