...
| `label` | `before-deploy-<7-character SHA>` | The Mark's name. It is lowercased and every run of characters other than letters, digits, `-` and `_` becomes `-`, cut to 63 characters. When the default name is taken (a re-run), `-2`, `-3`, ... up to `-20` is added. A `label` you set that is taken fails the step. |
...
| Mark not saved | Anything else: a `label` that is taken, a read-only key, the API unreachable. |
