...
2. **Between tasks only.** It never interrupts a backup, a Proof run or a Mark.
...
Running the installer again is safe. It changes only what differs, keeps your configuration, and doesn't restart the agent while a backup or Proof is running:
...
**After rotating bucket keys,** restart the agent, then click **Check again** on each database's Health page (or run `rowsafe verify app` for each database on the server). This rewrites the pgBackRest configuration that PostgreSQL's `archive_command` uses, and proves the new keys work.
...
```sh
rowsafe remove app --keep-archiving   # for each database on it
